 ...
 * for high-quality chapter text extraction from any novel website.
 */
fun injectNovelExtractorScript(webView: WebView) {
    try {
        val jsContent = webView.context.assets.open("novel_extractor.js")
            .bufferedReader().use { it.readText() }
        webView.evaluateJavascript(jsContent, null)
    } catch (e: Exception) {
        android.util.Log.w("NovelExtractor", "Failed to inject novel_extractor.js: ${e.message}")
    }
}

/**
 * Extract chapter paragraphs using the NovelExtractor JS library.
 * Falls back to the legacy extraction method if NovelExtractor is not available.
 *
 * @return JSON string array of paragraph texts
 */
fun extractParagraphsWithNovelExtractor(webView: WebView): String {
    // Use NovelExtractor if available, fallback to legacy method
    val jsCode = """
        (function() {
            if (window.NovelExtractor && window.NovelExtractor.extractParagraphs) {
                try {
                    return window.NovelExtractor.extractParagraphs();
                } catch(e) {
                    console.log('NovelExtractor.extractParagraphs failed: ' + e.toString());
                }
            }
            ...
            for (var i = 0; i < pTags.length; i++) {
                var text = pTags[i].innerText.trim();
                if (text.length > 5) {
                    pTags[i].setAttribute('wtr-translation-id', paragraphs.length);
            ...
            if (paragraphs.length === 0) {
                ...
                for (var i = 0; i < divs.length; i++) {
                    ...
                    if (text.length > 20) {
                        divs[i].setAttribute('wtr-translation-id', paragraphs.length);
                        paragraphs.push(text);
                    }
                }
            }
